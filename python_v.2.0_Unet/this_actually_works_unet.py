...
class FineTunedSequenceUNET(Model):
    def __init__(self, base_model, num_filters, dropout_rate=0.2):
        super(FineTunedSequenceUNET, self).__init__()
        self.base_model = base_model
        self.conv1d = Conv1D(num_filters, 3, activation='relu')  # Using Conv1D before flattening
        self.dropout = Dropout(dropout_rate)
        self.bn1 = BatchNormalization()
        self.flatten = Flatten()  # Move flatten after Conv1D operations
        self.dense = Dense(1, activation='sigmoid')  # Output layer for binary classification
    ...
    def call(self, inputs, training=False):
        ...
        x = self.flatten(x)  # Flatten here before the final dense layer
        return self.dense(x)
