...
def one_hot_encode(sequence):
    ...
    aa_to_index = {aa: idx for idx, aa in enumerate(amino_acids)}
    encoded = np.zeros((len(sequence), len(amino_acids)))  # Now 21 categories
...
class FineTunedSequenceUNET(Model):
    def __init__(self, base_model):
        ...
        # Adjust the first layer to accept 21 features instead of 20
        # Ensure the first layer of base_model is replaceable and set properly
        new_input_shape = (None, 21)  # Adjust the input shape for 21 features
        new_first_layer = Conv1D(filters=base_model.layers[0].filters,
                                 kernel_size=base_model.layers[0].kernel_size,
                                 activation='relu',
                                 input_shape=new_input_shape)
        
        # Rebuild the model with the new input layer
        self.base_model = Model(inputs=Input(shape=new_input_shape),
                                outputs=self.base_model(new_first_layer.output))
...
base_model = models.load_trained_model(model='patho_finetune', download=True)
final_model = FineTunedSequenceUNET(base_model, num_filters=num_filters, dropout_rate=0.5)
...
history = final_model.fit(train_dataset, epochs=100, validation_data=test2_dataset, callbacks=[early_stopping, lr_scheduler])
